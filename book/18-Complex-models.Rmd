...
```{r,eval = TRUE, echo = F, message = F, warning = F, fig.cap = " A scatter plot of longevity against body size across three treatments of differening male sexual activity. Fitted model slopes are from the reduced linear model (main effects only of thorax size, sleep and treatment group), with 95% confidence intervals"}

colours <- c("cyan", "darkorange", "purple")

emmeans::emmeans(flyls_sqrt2, # model
                 specs = ~ type + thorax, 
                 at = list(thorax = c(0.6, 0.7, 0.8, 0.9)), 
                 type = "response") %>% 
                 as_tibble() %>% 
                 ggplot(aes(x=thorax, 
                 y = response, 
